...
function InventoryPage() {
  ...
  const prods = (products.data ?? []) as Array<{
    id: string;
    name: string;
    slug: string;
    stock: number | null;
  ...
  return (
    <AdminShell title="Inventory">
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        <Card>
          <div className="text-xs uppercase text-muted-foreground">Total SKUs</div>
          <div className="mt-1 text-2xl font-bold">{prods.length}</div>
        </Card>
        <Card>
          <div className="text-xs uppercase text-muted-foreground">Low stock</div>
          <div className="mt-1 text-2xl font-bold text-amber-600">{low.length}</div>
        </Card>
        <Card>
          <div className="text-xs uppercase text-muted-foreground">Out of stock</div>
          <div className="mt-1 text-2xl font-bold text-rose-600">{out.length}</div>
        </Card>
      </div>
              ...
              <tr key={p.id}>
                ...
                <td className="p-3">
                  {(p.stock ?? 0) === 0 ? (
                    ...
                    <StatusPill tone="warn">{p.stock}</StatusPill>
                  ) : (
                    <span>{p.stock}</span>
                  )}
                </td>
                <td className="p-3">
                  <Input
                    ...
                    placeholder="0"
                    value={deltas[p.id] ?? ""}
                    onChange={(e) => setDeltas((s) => ({ ...s, [p.id]: e.target.value }))}
                    className="w-28"
                  />
                </td>
                <td className="p-3">
                  <div className="flex flex-wrap gap-1">
                    <Btn variant="primary" onClick={() => handle(p.id, "add")}>
                      + Add
                    </Btn>
                    <Btn variant="secondary" onClick={() => handle(p.id, "remove")}>
                      − Remove
                    </Btn>
                    <Btn variant="ghost" onClick={() => handle(p.id, "set")}>
                      Set total
                    </Btn>
                  </div>
                </td>
          ...
          <tbody className="divide-y divide-border">
            {(
              ...
              <tr key={m.id}>
                ...
                <td
                  className={`p-3 font-semibold ${m.delta < 0 ? "text-rose-600" : "text-emerald-600"}`}
                >
                  {m.delta > 0 ? "+" : ""}
                  {m.delta}
                </td>
