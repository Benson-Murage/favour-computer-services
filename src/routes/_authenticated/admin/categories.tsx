...
type Cat = { id: string; slug: string; name: string; icon: string | null; sort_order: number | null };
...
function CategoriesPage() {
  ...
  return (
    <AdminShell title="Categories">
      <div className="mb-4"><Btn onClick={()=>{ setEdit(null); setOpen(true); }}>+ Add category</Btn></div>
      <Card className="overflow-x-auto p-0">
        <table className="w-full text-sm">
          <thead className="bg-secondary text-left text-[11px] uppercase tracking-wider text-muted-foreground">
            <tr><th className="p-3">Name</th><th className="p-3">Slug</th><th className="p-3">Sort</th><th className="p-3"></th></tr>
          </thead>
...
function CatForm({ initial, onSave }: { initial: Cat | null; onSave: (p: { id?: string; name: string; slug: string; icon?: string; sort_order?: number }) => void }) {
  ...
  return (
    <form onSubmit={(e)=>{ e.preventDefault(); onSave({ id: initial?.id, name, slug, sort_order: Number(sort) || 0 }); }} className="grid gap-3">
      ...
      <Field label="Sort order"><Input type="number" value={sort} onChange={(e)=>setSo(e.target.value)} /></Field>
