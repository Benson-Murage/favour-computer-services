  ...
  return (
    <AdminShell title="Service Packages">
      ...
      <PkgTable
        ...
        onDelete={async (p) => {
          if (confirm("Delete?")) {
            ...
            qc.invalidateQueries({ queryKey: ["adm", "pkgs"] });
          }
        }}
      />
      <div className="h-6" />
      <PkgTable
        title="Live Streaming Packages"
        pkgs={live}
        onEdit={(p) => {
          setEdit(p);
          setOpen(true);
      ...
      <Modal
        ...
        title={edit ? `Edit · ${edit.name}` : "New package"}
      >
        <PkgForm
          initial={edit}
          onSave={async (p) => {
            ...
            {pkgs.map((p) => (
              <tr key={p.id}>
                ...
                <td className="p-3 text-right">
                  ...
                  <Btn variant="danger" className="ml-2" onClick={() => onDelete(p)}>
                    Delete
                  </Btn>
                </td>
              </tr>
