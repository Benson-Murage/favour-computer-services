...
function PaymentsAdmin() {
  ...
  return (
    <AdminShell title="Payments">
          ...
          <tbody className="divide-y divide-border">
            {((data ?? []) as Pay[]).map((p) => (
              <tr key={p.id}>
                ...
                <td className="p-3 font-mono text-xs">{p.order?.invoice_number ?? p.order_id.slice(0,8)}</td>
                <td className="p-3 font-semibold">{formatPrice(Number(p.amount))}</td>
                <td className="p-3 text-xs">{p.method || "M-Pesa"}{p.reference ? <div className="text-muted-foreground">{p.reference}</div> : null}</td>
                <td className="p-3"><StatusPill tone={p.status==="approved"?"success":p.status==="rejected"?"danger":"warn"}>{p.status}</StatusPill></td>
      ...
      <Modal open={!!open} onClose={() => setOpen(null)} title={`Review payment — ${open?.order?.invoice_number ?? ""}`}>
        {open && (
          <div className="space-y-3 text-sm">
            <div className="grid gap-2 sm:grid-cols-3 text-xs">
              <div><div className="text-muted-foreground">Amount</div><div className="font-semibold">{formatPrice(Number(open.amount))}</div></div>
              <div><div className="text-muted-foreground">Method</div><div className="font-semibold">{open.method || "M-Pesa"}</div></div>
              <div><div className="text-muted-foreground">Reference</div><div className="font-semibold">{open.reference || "—"}</div></div>
            </div>
            {open.proof_path && proofUrl && (open.proof_mime?.startsWith("image/")
              ? <img src={proofUrl} alt="Proof" className="max-h-[420px] w-full rounded-xl border border-border object-contain" />
              : <a href={proofUrl} target="_blank" rel="noreferrer" className="inline-block rounded-full bg-foreground px-4 py-2 text-xs font-semibold text-background">Open proof file</a>)}
            <div>
              <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Notes (sent to customer)</div>
              <Textarea rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <Btn variant="danger" onClick={() => act("reject")}>Reject</Btn>
