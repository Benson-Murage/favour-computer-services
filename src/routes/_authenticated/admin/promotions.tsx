  ...
  return (
    <AdminShell title="Promotions">
            ...
            {((data ?? []) as Promo[]).map((p) => {
              ...
              return (
                <tr key={p.id}>
                  ...
                  <td className="p-3 text-xs">
                    {p.active && !expired ? "Active" : expired ? "Expired" : "Inactive"}
                  </td>
      ...
      <Modal
        ...
        title={edit ? "Edit promotion" : "New promotion"}
      >
        <PromoForm
          initial={edit}
          products={(products.data ?? []) as Array<{ id: string; name: string }>}
          onSave={async (p) => {
            try {
