...
function UsersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return arr.filter((u) => u.email.toLowerCase().includes(s) || (u.full_name ?? "").toLowerCase().includes(s));
  }, [data, q]);

  const invalidate = () => qc.invalidateQueries({ queryKey: ["adm","users"] });
  const wrap = async (fn: () => Promise<unknown>, ok: string) => {
    try { await fn(); toast.success(ok); invalidate(); } catch (e) { toast.error((e as Error).message); }
  };

  return (
    <AdminShell title="User Management">
      <p className="text-sm text-muted-foreground">Manage accounts, roles, and access — no database edits required. Super-admins can change Admin/Super Admin roles; admins can manage Staff and Customer roles.</p>
          ...
          <tbody className="divide-y divide-border">
            ...
            {filtered.map((u) => (
              <tr key={u.id} className="hover:bg-secondary/40">
                ...
                <td className="p-3">
                  <Select value={u.role} onChange={(e)=>wrap(()=>setRole({ data: { user_id: u.id, role: e.target.value as Row["role"] } }), "Role updated")}>
                ...
                <td className="p-3">
                  {u.disabled ? <StatusPill tone="danger">Disabled</StatusPill> : <StatusPill tone="success">Active</StatusPill>}
                </td>
                <td className="p-3 whitespace-nowrap text-xs text-muted-foreground">{new Date(u.created_at).toLocaleDateString()}</td>
                <td className="p-3 whitespace-nowrap text-xs text-muted-foreground">{u.last_login_at ? new Date(u.last_login_at).toLocaleString() : "—"}</td>
                <td className="p-3">
            ...
            {!isLoading && filtered.length===0 && <tr><td colSpan={6} className="p-6 text-center text-muted-foreground">No users.</td></tr>}
          </tbody>
        </table>
      </Card>
    </AdminShell>
