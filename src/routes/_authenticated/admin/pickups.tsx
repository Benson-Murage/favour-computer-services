...
type Order = { id: string; created_at: string; customer_name: string; customer_email: string; customer_phone: string; fulfillment: string; total: number; status: "pending"|"paid"|"ready"|"picked_up"|"delivered"|"cancelled"; payment_status: "unpaid"|"awaiting_verification"|"paid"|"refunded"|null; reservation_number: string | null; pickup_code: string | null };
  ...
  const rows = useMemo(() => {
    ...
    const filtered = list.filter((o) => {
      ...
      if (search) {
        ...
        const hay = `${o.customer_name} ${o.customer_email} ${o.customer_phone} ${o.reservation_number ?? ""} ${o.pickup_code ?? ""}`.toLowerCase();
    ...
    <AdminShell title="Store Pickups">
      <div className="mb-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-6">
        <Select value={status} onChange={(e)=>setStatus(e.target.value as typeof STATUS_OPTIONS[number])}>
          {STATUS_OPTIONS.map((s)=><option key={s} value={s}>Status: {s.replace(/_/g," ")}</option>)}
        </Select>
        <Select value={payment} onChange={(e)=>setPayment(e.target.value as typeof PAYMENT_OPTIONS[number])}>
          {PAYMENT_OPTIONS.map((s)=><option key={s} value={s}>Payment: {s.replace(/_/g," ")}</option>)}
        </Select>
        <Input placeholder="Search customer / reservation" value={search} onChange={(e)=>setSearch(e.target.value)} />
