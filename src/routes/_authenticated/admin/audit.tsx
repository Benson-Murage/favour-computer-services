...
type Entry = { id: string; created_at: string; admin_email: string | null; action: string; entity: string; entity_id: string | null; details: Record<string, unknown> | null };
          ...
          <tbody className="divide-y divide-border">
            {((data ?? []) as Entry[]).map((e) => (
              <tr key={e.id}>
                ...
                <td className="p-3 text-xs">{e.entity}{e.entity_id ? ` · ${e.entity_id.slice(0,8)}` : ""}</td>
                <td className="p-3 text-[11px] text-muted-foreground"><code>{e.details ? JSON.stringify(e.details).slice(0, 120) : ""}</code></td>
              </tr>
            ))}
          </tbody>
        </table>
