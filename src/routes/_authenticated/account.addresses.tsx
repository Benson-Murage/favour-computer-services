...
function Addresses() {
  ...
  const load = useCallback(() => { list({}).then((d) => setRows(d as Address[])).catch((e) => toast.error((e as Error).message)); }, [list]);
  useEffect(() => { load(); }, [load]);
  ...
  const remove = async (id: string) => {
    ...
    try { await del({ data: { id } }); toast.success("Address deleted"); load(); }
    catch (e) { toast.error((e as Error).message); }
  };

  const makeDefault = async (id: string) => {
    try { await setDef({ data: { id } }); load(); } catch (e) { toast.error((e as Error).message); }
  };
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {!draft && <Btn onClick={() => setDraft({ ...empty })}><Plus className="mr-1 h-3.5 w-3.5" />Add address</Btn>}
      </div>
      ...
      {draft && (
        <Card className="mt-6">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <Field label="Label"><Input value={draft.label ?? ""} onChange={(e) => setDraft({ ...draft, label: e.target.value })} placeholder="Home, Office…" /></Field>
            <Field label="Recipient name"><Input value={draft.recipient_name ?? ""} onChange={(e) => setDraft({ ...draft, recipient_name: e.target.value })} /></Field>
            ...
            <Field label="Street address *" className="sm:col-span-2"><Input value={draft.line1} onChange={(e) => setDraft({ ...draft, line1: e.target.value })} /></Field>
            <Field label="Apt / suite" className="sm:col-span-2"><Input value={draft.line2 ?? ""} onChange={(e) => setDraft({ ...draft, line2: e.target.value })} /></Field>
            <Field label="City"><Input value={draft.city ?? ""} onChange={(e) => setDraft({ ...draft, city: e.target.value })} /></Field>
            <Field label="Region / county"><Input value={draft.region ?? ""} onChange={(e) => setDraft({ ...draft, region: e.target.value })} /></Field>
            <Field label="Postal code"><Input value={draft.postal_code ?? ""} onChange={(e) => setDraft({ ...draft, postal_code: e.target.value })} /></Field>
            ...
            <Card key={a.id} className="flex flex-wrap items-start justify-between gap-4">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {a.is_default && <span className="rounded-full bg-secondary px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider">Default</span>}
                </div>
              ...
              <div className="flex gap-2">
                {a.is_default ? (
                  <Btn variant="ghost" disabled><StarOff className="mr-1 h-3.5 w-3.5" />Default</Btn>
                ) : (
                  <Btn variant="secondary" onClick={() => makeDefault(a.id)}><Star className="mr-1 h-3.5 w-3.5" />Make default</Btn>
                )}
                <Btn variant="secondary" onClick={() => setDraft(a)}><Pencil className="mr-1 h-3.5 w-3.5" />Edit</Btn>
                <Btn variant="danger" onClick={() => remove(a.id)}><Trash2 className="mr-1 h-3.5 w-3.5" />Delete</Btn>
              </div>
            </Card>
          ))
        )}
