...
function OrderDetail() {
  ...
  const reload = () =>
    fetchOrder({ data: { id } })
      ...
      .catch((e) => {
        ...
        if (/not found|no rows|permission|Invalid uuid/i.test(msg)) {
          setNotFound(true);
        } else {
          toast.error(msg || "Failed to load order");
        }
      });
  useEffect(() => {
    ...
    return (
      <div className="mx-auto max-w-lg px-4 py-24 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          We couldn't find this order under your account. It may belong to a different email, or the
          link may be incorrect.
        </p>
        <div className="mt-6 flex justify-center gap-2">
          <Btn variant="secondary" onClick={() => navigate({ to: "/account/orders" })}>
            Back to My Orders
          </Btn>
  ...
  const submit = async (
    ...
    if (!file && !reference.trim()) {
      toast.error("Add a transaction code OR upload a screenshot");
      return;
    }
    setBusy(true);
    try {
      ...
      if (file) {
        ...
        const { error: upErr } = await supabase.storage
          .from("payment-proofs")
          .upload(proof_path, file, { contentType: file.type, upsert: false });
        if (upErr) throw upErr;
      }
      const finalRef = note.trim()
  ...
  const viewProof = async (path: string) => {
    if (proofUrls[path]) {
      window.open(proofUrls[path], "_blank");
      return;
    }
    ...
    <div className="mx-auto max-w-5xl px-4 py-10">
      ...
      <div className="mt-3 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">
            Order {order.invoice_number ?? `#${order.id.slice(0, 8).toUpperCase()}`}
          </h1>
        ...
        <div className="flex gap-2">
          <Btn
            variant="secondary"
            onClick={() => {
              void printReceiptPdf(receipt, biz);
            }}
          >
          ...
          <Btn
            variant="secondary"
            onClick={() => {
              void downloadReceiptPdf(receipt, biz, "invoice");
            }}
          >
          ...
          <Btn
            onClick={() => {
              void downloadReceiptPdf(receipt, biz, "receipt");
            }}
          >
          ...
          <Link
            to="/receipts/$id"
            params={{ id: order.id }}
            className="inline-flex h-9 items-center rounded-full border border-border bg-card px-3 text-xs font-semibold hover:bg-secondary"
          >
            View receipt
          </Link>
        </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-xs">
        ...
        {order.reservation_number && (
          <StatusPill tone="info">Reservation: {order.reservation_number}</StatusPill>
        )}
      </div>
        ...
        <section className="rounded-2xl border border-border bg-card p-6">
          ...
          <ul className="mt-3 divide-y divide-border text-sm">
            {items.map((it, i) => (
              <li key={i} className="flex justify-between py-2">
                <span>
                  {it.name} <span className="text-xs text-muted-foreground">× {it.qty}</span>
                </span>
          ...
          <ul className="mt-3 space-y-2 text-sm">
            <li className="flex gap-2">
              ...
              <span>{new Date(order.created_at).toLocaleString("en-KE")} — Order placed</span>
            </li>
            {notifications.map((n, i) => (
              <li key={i} className="flex gap-2">
                <Clock className="mt-0.5 h-4 w-4 text-muted-foreground" />
                <span>
                  {new Date(n.created_at).toLocaleString("en-KE")} — {n.subject}
                </span>
              </li>
            ))}
          </ul>
        ...
        <aside className="space-y-4">
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  {order.reservation_number && (
                    <div className="mt-2">
                      Reservation:{" "}
                      <span className="font-mono font-bold">{order.reservation_number}</span>
                    </div>
                  )}
                  {order.pickup_code && (
                    <div>
                      Pickup code: <span className="font-mono font-bold">{order.pickup_code}</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          {order.fulfillment === "delivery" && (
            <div className="rounded-2xl border border-border bg-card p-5 text-sm">
              ...
              {order.delivery_address && (
                <div className="mt-2 text-muted-foreground">{order.delivery_address}</div>
              )}
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            >
              ...
              <Field label="Method">
                <Input name="method" defaultValue="M-Pesa" />
              </Field>
              <Field label="M-Pesa transaction code / reference">
                <Input name="reference" placeholder="e.g. SLM7XYZ12" />
              </Field>
              <Field label="Payment message / note (optional)">
                <Input name="note" placeholder="Paste M-Pesa SMS message or any note" />
              </Field>
              ...
              <Btn type="submit" disabled={busy} className="w-full !h-10">
                <Upload className="mr-1 h-3.5 w-3.5" />
                {busy ? "Uploading…" : "Submit proof"}
              </Btn>
              {settings?.whatsapp && (
                <a
                  href={`https://wa.me/${settings.whatsapp.replace(/\D/g, "")}?text=${encodeURIComponent(`Hi, I just paid for order ${order.invoice_number ?? order.id.slice(0, 8)}.`)}`}
                  target="_blank"
                  rel="noreferrer"
              ...
              <ul className="mt-3 space-y-3 text-sm">
                {payments.map((p) => (
                  <li key={p.id} className="rounded-xl border border-border p-3">
                    ...
                    <div className="mt-1">
                      {formatPrice(Number(p.amount))} · {p.method || "M-Pesa"}
