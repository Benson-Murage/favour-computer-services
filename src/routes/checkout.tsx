  ...
  if (items.length === 0 && !done) {
    return (
      <div className="mx-auto max-w-xl px-4 py-24 text-center">
        ...
        <Link to="/shop" className="mt-4 inline-flex h-10 items-center rounded-full bg-foreground px-5 text-sm font-semibold text-background">Browse the shop</Link>
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-16">
        <div className="rounded-3xl border border-border bg-card p-8 text-center [box-shadow:var(--shadow-card)]">
          <div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-emerald-500/15 text-emerald-600 text-3xl">✓</div>
          <h1 className="mt-4 text-2xl font-bold">Order placed</h1>
          <p className="mt-1 text-sm text-muted-foreground">Reference: <span className="font-mono font-semibold">#{done.id.slice(0,8).toUpperCase()}</span></p>
          {done.reservation_number && (
            <div className="mx-auto mt-6 max-w-md rounded-2xl border border-border bg-secondary p-5 text-left">
              <div className="text-xs font-bold uppercase text-muted-foreground">Pickup Reservation</div>
              <div className="mt-1 font-mono text-xl font-bold">{done.reservation_number}</div>
              <div className="mt-1 text-xs text-muted-foreground">Pickup code: <span className="font-mono font-semibold">{done.pickup_code}</span></div>
              <div className="mt-3 flex items-start gap-2 text-sm">
                <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-[color:var(--accent)]" />
                <span>{settings?.pickup_location ?? "F&F Building, Shop U13, Next to Odeon Cinema, Nairobi"}</span>
              </div>
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          <section className="rounded-2xl border border-border bg-card p-6">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <button type="button" onClick={()=>setMethod("delivery")} className={`flex items-start gap-3 rounded-xl border p-4 text-left ${method==="delivery"?"border-foreground bg-secondary":"border-border"}`}>
                <Truck className="mt-0.5 h-5 w-5" />
                <div><div className="font-semibold">Delivery</div><div className="text-xs text-muted-foreground">Within Nairobi & arranged countrywide</div></div>
              </button>
              <button type="button" onClick={()=>setMethod("pickup")} className={`flex items-start gap-3 rounded-xl border p-4 text-left ${method==="pickup"?"border-foreground bg-secondary":"border-border"}`}>
                <Store className="mt-0.5 h-5 w-5" />
                <div><div className="font-semibold">Store Pickup</div><div className="text-xs text-muted-foreground">{settings?.pickup_location ?? "F&F Building, Shop U13, Nairobi"}</div></div>
              </button>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-border bg-card p-6">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <Field label="Phone"><Input name="phone" required placeholder="07XX XXX XXX" /></Field>
            </div>
          ...
          <ul className="divide-y divide-border text-sm">
            {items.map((i) => (
              <li key={i.id} className="flex justify-between py-2">
                <span>{i.name} <span className="text-xs text-muted-foreground">× {i.qty}</span></span>
                <span className="font-semibold">{formatPrice(Number(i.price) * i.qty)}</span>
              </li>
            ))}
          </ul>
          <div className="my-2 h-px bg-border" />
          <div className="flex justify-between text-base font-bold"><span>Total</span><span>{formatPrice(subtotal)}</span></div>
        </aside>
      </div>
    </div>
  );
