  ...
  if (!user) {
    return (
      <div className="mx-auto max-w-md px-4 py-20 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Access orders, addresses, and saved items.</p>
    ...
    <div className="mx-auto max-w-5xl px-4 py-10">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold tracking-tight">Hi, {user.email?.split("@")[0]}</h1>
        </div>
        <button onClick={signOut} className="inline-flex h-10 items-center gap-2 rounded-full border border-border bg-card px-4 text-sm font-medium hover:bg-secondary">
...
function Tile({ to, Icon, t, s }: { to: string; Icon: React.ComponentType<{ className?: string }>; t: string; s: string }) {
  return (
    <Link to={to} className="group rounded-2xl border border-border bg-card p-5 transition hover:[box-shadow:var(--shadow-card)]">
