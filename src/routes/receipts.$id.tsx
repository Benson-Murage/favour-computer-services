  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 print:py-0">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
        <Link to={user ? "/account/orders" : "/"} className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-muted-foreground hover:text-foreground">
      ...
      <div className="overflow-hidden rounded-3xl border border-border bg-card [box-shadow:var(--shadow-elevated)] print:rounded-none print:border-0 print:shadow-none">
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="mt-1 text-xs text-muted-foreground">Tel: {biz.phone} · {biz.email}</div>
              </div>
            </div>
            <div className="text-right">
              <div className="text-[10px] font-bold uppercase tracking-[0.24em] text-muted-foreground">{kind}</div>
              <div className="mt-1 font-mono text-lg font-bold">
                {kind === "invoice" ? order.invoice_number ?? `INV-${order.id.slice(0, 8).toUpperCase()}` : receiptNo}
              </div>
              <div className="mt-2 text-xs text-muted-foreground">Date: {new Date(order.created_at).toLocaleDateString("en-KE")}</div>
              <div className="mt-2 inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-2.5 py-1 text-[10px] font-bold uppercase">
                {paid ? <><CheckCircle2 className="h-3 w-3 text-emerald-500" /> Paid</> : rejected ? <><XCircle className="h-3 w-3 text-rose-500" /> Refunded</> : <><Clock className="h-3 w-3 text-amber-500" /> {(order.payment_status ?? "unpaid").replace(/_/g, " ")}</>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-6 border-b border-border p-8 sm:grid-cols-2">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">Billed to</div>
            <div className="mt-2 text-sm font-semibold">{order.customer_name}</div>
            <div className="text-sm text-muted-foreground">{order.customer_email}</div>
            <div className="text-sm text-muted-foreground">{order.customer_phone}</div>
            {order.delivery_address && <div className="mt-2 text-sm text-muted-foreground">{order.delivery_address}</div>}
          </div>
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">Order details</div>
            <div className="mt-2 grid gap-1 text-sm">
              <div><span className="text-muted-foreground">Order:</span> <span className="font-mono font-semibold">#{order.id.slice(0, 8).toUpperCase()}</span></div>
              <div><span className="text-muted-foreground">Fulfillment:</span> {order.fulfillment}</div>
              {order.reservation_number && <div><span className="text-muted-foreground">Reservation:</span> <span className="font-mono font-semibold">{order.reservation_number}</span></div>}
              {order.pickup_code && <div><span className="text-muted-foreground">Pickup code:</span> <span className="font-mono font-semibold">{order.pickup_code}</span></div>}
            </div>
          </div>
        </div>
        ...
        <div className="p-8">
          ...
          {(biz.till_number || biz.paybill_number) && (
            <div className="mt-8 rounded-2xl border border-border bg-secondary/40 p-5">
              <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">Payment</div>
              <div className="mt-2 grid gap-1 text-sm">
                {biz.till_number && <div>M-Pesa Till: <span className="font-mono font-bold">{biz.till_number}</span></div>}
                {biz.paybill_number && <div>M-Pesa Paybill: <span className="font-mono font-bold">{biz.paybill_number}</span>{biz.account_number ? ` · Account ${biz.account_number}` : ""}</div>}
              </div>
              {payments.length > 0 && (
                <div className="mt-3 text-xs text-muted-foreground">
                  Latest payment: {formatPrice(Number(payments[0].amount))} via {payments[0].method || "M-Pesa"}
