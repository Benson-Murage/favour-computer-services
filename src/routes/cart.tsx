  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-24 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Start exploring deals and add something you love.</p>
        <Link to="/shop" className="mt-6 inline-flex h-11 items-center rounded-full bg-foreground px-6 text-sm font-semibold text-background">
          Continue shopping
        </Link>
      </div>
    );
        ...
        <ul className="divide-y divide-border overflow-hidden rounded-2xl border border-border bg-card">
          {items.map((i) => (
            <li key={i.id} className="flex gap-4 p-4">
              <Link to="/products/$slug" params={{ slug: i.slug }} className="h-24 w-24 shrink-0 overflow-hidden rounded-xl bg-secondary">
                {i.image_url && <img src={i.image_url} alt={i.name} className="h-full w-full object-cover" />}
              </Link>
              <div className="flex flex-1 flex-col">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <Link to="/products/$slug" params={{ slug: i.slug }} className="line-clamp-2 text-sm font-semibold hover:underline">
                      {i.name}
                    </Link>
                    <div className="mt-1"><ConditionBadge condition={i.condition} /></div>
                  </div>
                  <div className="text-right text-sm font-bold">{formatPrice(Number(i.price) * i.qty)}</div>
                </div>
                <div className="mt-auto flex items-center justify-between">
                  ...
                  <button onClick={() => remove(i.id)} className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-destructive">
...
function Row({ k, v, bold }: { k: string; v: string; bold?: boolean }) {
  return (
    <div className={`flex justify-between text-sm ${bold ? "text-base font-bold" : "text-foreground"}`}>
