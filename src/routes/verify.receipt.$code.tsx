...
export const Route = createFileRoute("/verify/receipt/$code")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Verify the authenticity of a Favour Computer Services receipt using its verification code." },
        ...
        {loading ? (
          ...
          <div className="p-6">
            <div className="flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-900 dark:border-emerald-800/40 dark:bg-emerald-900/20 dark:text-emerald-100">
              ...
              <div>
                ...
                <div className="text-xs opacity-80">Verified on {new Date().toLocaleString("en-KE")}</div>
              </div>
            </div>

            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <Row label="Verification Code" value={<span className="font-mono font-bold">{result.verification_code}</span>} />
              <Row label="Receipt Number" value={<span className="font-mono">{result.invoice_number ?? "—"}</span>} />
              <Row label="Date Issued" value={result.issued_at ? new Date(result.issued_at).toLocaleString("en-KE") : "—"} />
              ...
              <Row label="Order Status" value={<span className="capitalize">{(result.status ?? "").replace(/_/g, " ")}</span>} />
              <Row label="Payment Status" value={<span className="capitalize">{(result.payment_status ?? "").replace(/_/g, " ")}</span>} />
            </div>

            {items.length > 0 && (
              <div className="mt-6 rounded-2xl border border-border bg-secondary/30 p-4">
                <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">Purchased items</div>
                <ul className="mt-3 divide-y divide-border">
                  {items.map((it, i) => (
                    <li key={i} className="flex items-center justify-between py-2 text-sm">
                      <span>{it.name} <span className="text-muted-foreground">× {it.qty}</span></span>
                      <span className="font-mono">{formatPrice(Number(it.price ?? 0) * Number(it.qty ?? 0))}</span>
                    </li>
                  ))}
                </ul>
          ...
          <div className="p-6">
            <div className="flex items-center gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-rose-900 dark:border-rose-800/40 dark:bg-rose-900/20 dark:text-rose-100">
              ...
              <div>
                <div className="font-semibold">This receipt is invalid or could not be verified.</div>
                <div className="text-xs opacity-80">Please double-check the verification code and try again.</div>
              </div>
            </div>
            ...
            >
              <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Enter Verification Code</label>
              <div className="mt-2 flex gap-2">
                <Input value={manual} onChange={(e) => setManual(e.target.value)} placeholder="FCS-2026-XXXXXX" className="font-mono uppercase" />
                <Btn type="submit"><Search className="mr-1 h-3.5 w-3.5" />Verify</Btn>
              </div>
            </form>
          </div>
        )}
