...
export function PaymentCard({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-emerald-500/10 via-card to-card p-6 [box-shadow:var(--shadow-elevated)] ${compact ? "" : ""}`}
    >
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.22em] text-emerald-500">
              Lipa na M-Pesa
            </div>
          ...
          <ol className="mt-3 grid gap-2 text-sm sm:grid-cols-2">
            {[
              ...
              tillNumber
                ...
                : `Enter Business No. ${paybillNumber}${accountNumber ? ` and Account ${accountNumber}` : ""}.`,
              typeof amount === "number" && amount > 0
                ? `Enter amount KES ${amount.toLocaleString("en-KE")} and confirm.`
                : "Enter the amount and confirm.",
            ].map((step, i) => (
              <li key={i} className="flex items-start gap-2.5">
                <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-foreground text-[10px] font-bold text-background">
                  {i + 1}
                </span>
                <span>{step}</span>
              </li>
...
function PayLine({
  ...
  return (
    ...
    >
      ...
      <span
        className={`grid h-9 w-9 shrink-0 place-items-center rounded-full border transition ${copied ? "border-emerald-500 bg-emerald-500 text-white" : "border-border bg-secondary text-muted-foreground group-hover:text-foreground"}`}
      >
