...
export const listUsers = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return list.users.map((u) => {
      const prof = profMap.get(u.id) as { full_name?: string | null; phone?: string | null; disabled?: boolean; last_login_at?: string | null } | undefined;
      const userRoles = roleMap.get(u.id) ?? [];
      const top: AppRole = userRoles.includes("super_admin") ? "super_admin"
...
export const setUserRole = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await logAudit(context.supabase, {
      ...
      action: "set_role", entity: "user", entityId: data.user_id, details: { role: data.role },
    });
    return { ok: true };
  });
...
export const setUserDisabled = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await logAudit(context.supabase, {
      adminId: context.userId, adminEmail: context.claims?.email ?? "",
      action: data.disabled ? "disable" : "enable", entity: "user", entityId: data.user_id,
    });
    return { ok: true };
  });
...
export const sendPasswordReset = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await logAudit(context.supabase, {
      adminId: context.userId, adminEmail: context.claims?.email ?? "",
      action: "reset_password", entity: "user", details: { email: data.email },
    });
...
export const forceSignOut = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await logAudit(context.supabase, {
      adminId: context.userId, adminEmail: context.claims?.email ?? "",
      action: "force_signout", entity: "user", entityId: data.user_id,
    });
    return { ok: true };
  });
